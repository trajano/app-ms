  ...
  >
    ...
    <div class="layout-padding logo-container non-selectable no-pointer-events">
      <div class="logo" :style="position">
              <div>
          <div>Hello world {{accessToken}}</div>
          <div> {{data.principal}}</div>
          <div>{{data.claims.iat}}</div>
              </div>
...
export default {
  ...
  async created() {
    this.data = await this.$store.getters.secureGet('/v1/secure')
  },
  computed: {
    accessToken() {
  ...
  methods: {
    ...
    signoff() {
      this.$store.dispatch('signoff', this.$router)
